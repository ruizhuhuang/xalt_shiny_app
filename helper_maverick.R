...
log_data_dir = "/work/03076/rhuang/wrangler/xalt/data"
...
runXaltJava = function(StartMonth, EndMonth,conf, sup,output,
  ...
  submit_cmd_1 = sprintf("/home/03076/rhuang/spark-1.6.0-bin-hadoop2.6/bin/spark-submit --class xalt.xalt --master local[16]  /work/03076/rhuang/wrangler/xalt/xalt2.jar -b %s -v %s,%s -f file:%s/xalt_tmp/ -out %s/xalt_out -d ",
                       b,v,b,
                       Sys.getenv("HOME"), Sys.getenv("HOME"))

  
#  --executor-cores 1
  submit_cmd_2 = sprintf("/home/03076/rhuang/spark-1.6.0-bin-hadoop2.6/bin/spark-submit --class xalt.xalt --master local[16]  /work/03076/rhuang/wrangler/xalt/xalt2.jar -a -b %s -v %s -conf %s -sup %s -f file:%s/xalt_tmp/ -out %s/xalt_out ",
                         as_b, as_v_b,
...
bar_plot = function(top_n_apps,column_dis_b, column_dis_v){
  ...
  if (column_dis_v=="field_of_science" | column_dis_b=="field_of_science" ){
    fields= read.table( "/work/03076/rhuang/wrangler/xalt/fields.txt", sep="\t", row.names=1)
